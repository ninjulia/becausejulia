...
const gridInner = document.querySelector('#latestGrid');
...
//
 jribbble.shots(function(shotsArray) {
  ...
  shotsArray.forEach(shot => {
    const slideImage = `<div class="grid-item"><a class="stretched-link" href="${shot.html_url}" target="_blank"><img src="${shot.images.normal}" class="img-fluid rounded-1" alt="${shot.title}"></a></div>`;
...
jribbble.user(function(userObj) {
  const gridSubText = `<p class="text-end">View more posts from <a href="${userObj.html_url}" target="_blank" class="text-white">${userObj.login} on dribbble.</a></p>`;
